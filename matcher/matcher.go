...
func GenerateConnectToken(clientId uint64, serverAddresses []net.UDPAddr, protocolId uint64, expireSeconds uint64, timeoutSeconds int32, userData []byte, privateKey []byte) ([]byte) {
    ...
    log.Printf( "Generating client token for %d\n", clientId )
...
func MatchHandler( w http.ResponseWriter, r * http.Request ) {
    ...
    log.Printf( "for %s : %d", ServerAddress, ServerPort )
...
//    http.HandleFunc("/hello", HelloServer)
    ...
    defMux.Handle("/students", sHandler)

    r := mux.NewRouter()
    r.HandleFunc("/", HelloServer)
    r.HandleFunc("/students", HelloServer)
